...
def process_bible_files(pdf_directory='./'):
    """
    ...
    all_verses = []
    pdf_files = list(Path(pdf_directory).glob('NLT_*.pdf'))
    ...
    # Group verses for better semantic search
    ...
    # Save individual verses (for reference/lookup)
    with open('bible_verses.json', 'w', encoding='utf-8') as f:
        json.dump(all_verses, f, ensure_ascii=False, indent=2)
    print("✅ bible_verses.json saved")
    
    # Save grouped verses (for embedding)
    with open('bible_grouped.json', 'w', encoding='utf-8') as f:
        json.dump(grouped, f, ensure_ascii=False, indent=2)
    print("✅ bible_grouped.json saved")
    
    # Save embedding-ready format (for Pinecone)
    with open('bible_for_embedding.json', 'w', encoding='utf-8') as f:
    ...
    print("\nNext steps:")
    print("1. Upload bible_for_embedding.json to Pinecone")
    print("2. Update server.py to cite Bible verses")

if __name__ == '__main__':
    # Run from current directory (where PDFs are located)
    process_bible_files('.')